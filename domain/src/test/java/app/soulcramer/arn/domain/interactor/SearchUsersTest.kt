...
class SearchUsersTest {
    ...
    @Test
    fun `Given empty search nickname When searching corresponding the user Then return all users`() {
        runBlocking {
    ...
    @Test
    fun `Given a search nickname When searching non existent users Then return empty list`() {
        runBlocking {
    ...
    @Test
    fun `Given a search nickname When searching existent users Then return list of users`() {
        runBlocking {

            val searchedNickname = "abcd"
            coEvery {
                testUserRepository.searchUser(searchedNickname)
            } returns UserFactory.makeUserListWithCloseNickname(5, searchedNickname)
